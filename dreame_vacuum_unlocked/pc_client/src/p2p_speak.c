/* p2p_speak - stream audio to a Dreame vacuum via the Tencent XP2P
 * talk-back (voice intercom) channel.
 *
 * Reuses the vendored Tencent XP2P SDK wrapper (p2p_api.c/.h, appWrapper.h).
 * Establishes the same WAN P2P link the live camera uses, then pushes audio
 * bytes to the device speaker through the SDK's "send voice" service
 * (runSendService + dataSend).
 *
 * Usage:  XP2P_INFO=<p2p_info> ./p2p_speak <config.txt> <raw_audio.bin> [channel] [crypto]
 */
...
int main(int argc, char **argv)
{
    ...
    /* Probe whether the device advertises the "voice" (audio) capability. */
    {
    ...
    if (!fp) { perror("fopen audio"); return 1; }
